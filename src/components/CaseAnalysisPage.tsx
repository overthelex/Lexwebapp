...
import { FileText, Users, Scale, BookOpen, TrendingUp, Calendar, CheckCircle, Loader, ArrowLeft, Network, Clock } from 'lucide-react';
...
export function CaseAnalysisPage({
  ...
  const relatedDecisions: RelatedDecision[] = [{
    ...
    date: '15.11.2023',
    similarity: 95,
    summary: 'Взыскание задолженности по договору поставки с учетом неустойки'
  }, {
    id: '2',
  ...
  if (isLoading) {
    return <div className="flex-1 h-full flex items-center justify-center bg-claude-bg p-8">
        <motion.div initial={{
        opacity: 0,
        scale: 0.9
      }} animate={{
            ...
            <div className="space-y-4">
              ...
              return <motion.div key={index} initial={{
                ...
                delay: index * 0.1
                    ...
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${isActive ? 'bg-claude-accent text-white' : isComplete ? 'bg-green-500 text-white' : 'bg-white text-claude-subtext border border-claude-border'}`}>
                      {isComplete ? <CheckCircle size={20} /> : <StageIcon size={20} />}
                    </div>
                    <div className="flex-1">
                      <p className={`text-sm font-medium font-sans ${isActive || isComplete ? 'text-claude-text' : 'text-claude-subtext'}`}>
                        {stage.text}
                      </p>
                    </div>
                  </motion.div>;
  ...
  return <div className="flex-1 h-full overflow-y-auto bg-claude-bg">
      {/* Header */}
      <div className="sticky top-0 z-10 bg-white border-b border-claude-border">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4">
          <div className="flex items-center gap-4">
            <button onClick={onBack} className="p-2 hover:bg-claude-bg rounded-lg transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 space-y-8">
        ...
        delay: 0.2
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={() => setShowGraph(!showGraph)} className="px-4 py-2 bg-claude-accent text-white rounded-xl text-sm font-medium font-sans hover:bg-[#C66345] transition-colors">
              {showGraph ? 'Скрыть граф' : 'Показать граф'}
            </button>
          </div>

          <AnimatePresence>
            {showGraph && <motion.div initial={{
            height: 0,
            opacity: 0
          }} animate={{
            height: 'auto',
            ...
            duration: 0.3
                ...
                return <motion.div key={decision.id} initial={{
                  ...
                  delay: index * 0.1 + 0.3
                }} className="absolute top-1/2 left-1/2" style={{
                  transform: `translate(calc(-50% + ${x}px), calc(-50% + ${y}px))`
                }}>
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center shadow-md ${decision.similarity > 85 ? 'bg-green-500' : decision.similarity > 70 ? 'bg-amber-500' : 'bg-gray-400'}`}>
          ...
          <div className="space-y-3">
            {relatedDecisions.map((decision, index) => <motion.div key={decision.id} initial={{
            opacity: 0,
            x: -20
          }} animate={{
            opacity: 1,
            ...
            delay: index * 0.05 + 0.3
                ...
                <div className="flex-shrink-0">
                  <div className={`w-16 h-16 rounded-xl flex flex-col items-center justify-center ${decision.similarity > 85 ? 'bg-green-100 text-green-700' : decision.similarity > 70 ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-700'}`}>
        ...
        delay: 0.3
          ...
          <div className="relative">
            ...
            <div className="space-y-6">
              {timeline.map((event, index) => <motion.div key={index} initial={{
              opacity: 0,
              x: -20
            }} animate={{
              opacity: 1,
              ...
              delay: index * 0.1 + 0.4
                  ...
                  <div className="flex-shrink-0 w-16">
                    <div className={`w-16 h-16 rounded-xl flex items-center justify-center font-serif font-bold text-lg ${event.impact === 'positive' ? 'bg-green-100 text-green-700' : event.impact === 'negative' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'}`}>
                      {event.year}
                    </div>
                  </div>
                  ...
                  <div className="flex-1 pb-6">
                    <div className="bg-claude-bg rounded-xl border border-claude-border p-4">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        <span className={`px-2 py-1 rounded text-xs font-medium font-sans ${event.impact === 'positive' ? 'bg-green-100 text-green-700' : event.impact === 'negative' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'}`}>
                          {event.impact === 'positive' ? 'Позитивно' : event.impact === 'negative' ? 'Негативно' : 'Нейтрально'}
                        </span>
                      </div>
                      <p className="text-sm text-claude-subtext font-sans">
                        {event.description}
