...
import { Search, Users, ChevronRight, Mail, Phone, Briefcase, Calendar, Plus, Filter, MoreVertical, MessageSquare, Send, X } from 'lucide-react';
interface Client {
  id: string;
  name: string;
  company: string;
...
export function ClientsPage({
  ...
  const filteredClients = clientsData.filter(client => {
    const matchesSearch = client.name.toLowerCase().includes(searchQuery.toLowerCase()) || client.company.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return <div className="flex-1 h-full overflow-y-auto bg-claude-bg p-4 md:p-8 lg:p-12 pb-32">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        ease: [0.22, 1, 0.36, 1]
          ...
          <div className="flex flex-col md:flex-row gap-3">
            <div className="relative flex-1 group">
              ...
              <input type="text" className="block w-full pl-11 pr-4 py-3 bg-white border border-claude-border rounded-xl text-claude-text placeholder-claude-subtext/50 focus:outline-none focus:ring-2 focus:ring-claude-accent/20 focus:border-claude-accent transition-all shadow-sm font-sans" placeholder="Поиск по имени или компании..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>

            <div className="flex gap-2">
              <button onClick={() => setFilterType('all')} className={`px-4 py-3 rounded-xl font-medium text-sm font-sans transition-all ${filterType === 'all' ? 'bg-claude-accent text-white shadow-sm' : 'bg-white text-claude-text border border-claude-border hover:bg-claude-bg'}`}>
                Все
              </button>
              <button onClick={() => setFilterType('individual')} className={`px-4 py-3 rounded-xl font-medium text-sm font-sans transition-all ${filterType === 'individual' ? 'bg-claude-accent text-white shadow-sm' : 'bg-white text-claude-text border border-claude-border hover:bg-claude-bg'}`}>
                Физ. лица
              </button>
              <button onClick={() => setFilterType('corporate')} className={`px-4 py-3 rounded-xl font-medium text-sm font-sans transition-all ${filterType === 'corporate' ? 'bg-claude-accent text-white shadow-sm' : 'bg-white text-claude-text border border-claude-border hover:bg-claude-bg'}`}>
                Юр. лица
              </button>
            </div>
          </div>
          ...
          {/* Select All */}
          {filteredClients.length > 0 && <div className="flex items-center gap-3">
              <label className="flex items-center gap-2 px-3 py-2 bg-white rounded-lg border border-claude-border hover:bg-claude-bg transition-colors cursor-pointer">
                <input type="checkbox" checked={selectedClients.size === filteredClients.length && filteredClients.length > 0} onChange={selectAll} className="w-4 h-4 rounded border-claude-border text-claude-accent focus:ring-claude-accent cursor-pointer" />
                <span className="text-sm font-medium text-claude-text font-sans">
                  Выбрать все
                </span>
              </label>
              {selectedClients.size > 0 && <span className="text-sm text-claude-subtext font-sans">
        ...
        }} onClick={() => onSelectClient?.(client)} className="group bg-white rounded-2xl p-5 border border-claude-border shadow-sm hover:shadow-md hover:border-claude-subtext/30 transition-all cursor-pointer relative">
              <div className="flex items-start gap-4">
                {/* Checkbox */}
                <div className="pt-1" onClick={e => e.stopPropagation()}>
                  <input type="checkbox" checked={selectedClients.has(client.id)} onChange={e => toggleClientSelection(client.id, e)} className="w-4 h-4 rounded border-claude-border text-claude-accent focus:ring-claude-accent cursor-pointer" />
                </div>
                ...
                <div className="w-12 h-12 rounded-full bg-claude-sidebar border-2 border-white shadow-sm flex items-center justify-center text-lg font-serif text-claude-subtext flex-shrink-0 relative">
                  ...
                  <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white ${client.status === 'active' ? 'bg-green-500' : 'bg-gray-400'}`} />
                </div>
      ...
      {/* Floating Action Bar */}
      <AnimatePresence>
        {selectedClients.size > 0 && <motion.div initial={{
        y: 100,
        opacity: 0
      }} animate={{
        y: 0,
            ...
            <div className="bg-claude-text text-white rounded-2xl shadow-2xl p-4 flex items-center gap-4">
              <div className="flex items-center gap-3 px-2">
                ...
                <span className="font-sans text-sm">
                  {selectedClients.size === 1 ? 'клиент выбран' : 'клиентов выбрано'}
                </span>
              </div>
              ...
              <div className="flex gap-2">
                <button onClick={handleSendMessage} className="flex items-center gap-2 px-4 py-2 bg-white text-claude-text rounded-xl font-medium text-sm font-sans hover:bg-gray-100 transition-colors">
                  <Send size={16} />
                  Отправить сообщение
                </button>
                <button onClick={() => setSelectedClients(new Set())} className="p-2 hover:bg-white/10 rounded-xl transition-colors">
                  <X size={20} />
                </button>
              </div>
            </div>
