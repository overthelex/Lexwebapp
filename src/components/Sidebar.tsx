...
import { Plus, MessageSquare, X, Users, Briefcase, FileText, Scale, Clock, Gavel, BookOpen, CheckCircle, LogOut, User } from 'lucide-react';
...
export function Sidebar({
  ...
  return <>
      ...
      {/* Sidebar Container */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-[280px] bg-claude-sidebar border-r border-claude-border flex flex-col transition-transform duration-400 ease-[cubic-bezier(0.22,1,0.36,1)] ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto px-3 py-1">
          ...
          <div className="mb-6">
            ...
            <div className="space-y-0.5">
              {contextSections.map(section => <button key={section.id} onClick={() => {
              if (section.onClick) {
                section.onClick();
                if (window.innerWidth < 1024) onClose();
              }
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    <section.icon size={15} strokeWidth={2} className="text-claude-subtext/60 group-hover:text-claude-text transition-colors duration-200 flex-shrink-0" />
                    <span className="truncate font-medium tracking-tight font-sans">
                      {section.label}
                    </span>
                  </div>
                  {section.count !== null && <span className="text-[11px] text-claude-subtext/50 font-medium px-1.5 py-0.5 bg-claude-subtext/5 rounded flex-shrink-0 font-sans">
                      {section.count}
                    </span>}
                </button>)}
            </div>
          ...
          <div className="mb-6">
            ...
            <div className="space-y-0.5">
              {evidenceSections.map(section => <button key={section.id} onClick={() => {
              if (section.onClick) {
                section.onClick();
                if (window.innerWidth < 1024) onClose();
              }
          ...
          <div className="mb-6">
            ...
            <div className="space-y-0.5">
              ...
              if (window.innerWidth < 1024) onClose();
                ...
                <Scale size={15} strokeWidth={2} className="text-claude-subtext/60 group-hover:text-claude-text transition-colors duration-200 flex-shrink-0" />
                <span className="truncate font-medium tracking-tight font-sans">
                  Судьи
                </span>
              </button>
              ...
              if (window.innerWidth < 1024) onClose();
                ...
                <Briefcase size={15} strokeWidth={2} className="text-claude-subtext/60 group-hover:text-claude-text transition-colors duration-200 flex-shrink-0" />
                <span className="truncate font-medium tracking-tight font-sans">
                  Адвокаты
                </span>
              </button>
        ...
        <div className="p-4 border-t border-claude-border relative" ref={profileMenuRef}>
          ...
          <AnimatePresence>
            ...
            ease: [0.22, 1, 0.36, 1]
                ...
                <button onClick={handleProfileClick} className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-claude-bg transition-colors border-b border-claude-border/50">
                ...
                <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-red-50 transition-colors">
          ...
          <button onClick={handleProfileMenuClick} className="w-full flex items-center gap-3 px-2 py-2 hover:bg-claude-subtext/8 rounded-lg transition-all duration-200">
