...
import { Search, Briefcase, Plus, Filter, Calendar, User, Scale, MoreVertical, Trash2, Edit, Eye, X, LayoutGrid, List, Send, ChevronDown } from 'lucide-react';
interface Case {
  id: string;
  number: string;
  title: string;
...
export function CasesPage({
  ...
  const filteredCases = casesData.filter(caseItem => {
    const matchesSearch = caseItem.number.toLowerCase().includes(searchQuery.toLowerCase()) || caseItem.title.toLowerCase().includes(searchQuery.toLowerCase()) || caseItem.client.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesStatus = filterStatus === 'all' || caseItem.status === filterStatus;
    const matchesClient = filterClient === 'all' || caseItem.client === filterClient;
  ...
  return <div className="flex-1 h-full overflow-y-auto bg-claude-bg p-4 md:p-8 lg:p-12 pb-32">
      <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex flex-col md:flex-row gap-3">
              <div className="relative flex-1 group">
                ...
                <input type="text" className="block w-full pl-11 pr-4 py-3 bg-white border border-claude-border rounded-xl text-claude-text placeholder-claude-subtext/50 focus:outline-none focus:ring-2 focus:ring-claude-accent/20 focus:border-claude-accent transition-all shadow-sm font-sans" placeholder="Поиск по номеру дела, названию или клиенту..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
              </div>

              <div className="flex gap-2">
                <button onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-2 px-4 py-3 rounded-xl font-medium text-sm font-sans transition-all ${showFilters ? 'bg-claude-accent text-white shadow-sm' : 'bg-white text-claude-text border border-claude-border hover:bg-claude-bg'}`}>
                  <Filter size={18} />
                  Фильтры
                ...
                <div className="flex bg-white border border-claude-border rounded-xl p-1">
                  <button onClick={() => setViewMode('comfortable')} className={`p-2 rounded-lg transition-colors ${viewMode === 'comfortable' ? 'bg-claude-accent text-white' : 'text-claude-subtext hover:text-claude-text'}`} title="Комфортный вид">
                    <LayoutGrid size={18} />
                  </button>
                  <button onClick={() => setViewMode('compact')} className={`p-2 rounded-lg transition-colors ${viewMode === 'compact' ? 'bg-claude-accent text-white' : 'text-claude-subtext hover:text-claude-text'}`} title="Компактный вид">
                    <List size={18} />
                  </button>
                </div>
              </div>
                  ...
                  <div className="bg-white rounded-xl p-4 border border-claude-border shadow-sm space-y-4">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 flex-wrap">
                        <button onClick={() => setFilterStatus('all')} className={`px-3 py-1.5 rounded-lg font-medium text-sm font-sans whitespace-nowrap transition-all ${filterStatus === 'all' ? 'bg-claude-accent text-white shadow-sm' : 'bg-claude-bg text-claude-text border border-claude-border hover:bg-claude-border'}`}>
                          Все дела
                        </button>
                        {Object.entries(statusConfig).map(([status, config]) => <button key={status} onClick={() => setFilterStatus(status as Case['status'])} className={`px-3 py-1.5 rounded-lg font-medium text-sm font-sans whitespace-nowrap transition-all ${filterStatus === status ? config.color : 'bg-claude-bg text-claude-text border border-claude-border hover:bg-claude-border'}`}>
                              {config.label}
                            </button>)}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-claude-border">
                      <div>
                        ...
                        <select value={filterClient} onChange={e => setFilterClient(e.target.value)} className="w-full px-4 py-2.5 bg-white border border-claude-border rounded-lg text-claude-text focus:outline-none focus:ring-2 focus:ring-claude-accent/20 focus:border-claude-accent transition-all font-sans">
                          <option value="all">Все клиенты</option>
                          {uniqueClients.map(client => <option key={client} value={client}>
                              {client}
                            </option>)}
                        </select>
                      </div>
                      ...
                      <div>
                        ...
                        <select value={filterJudge} onChange={e => setFilterJudge(e.target.value)} className="w-full px-4 py-2.5 bg-white border border-claude-border rounded-lg text-claude-text focus:outline-none focus:ring-2 focus:ring-claude-accent/20 focus:border-claude-accent transition-all font-sans">
                          <option value="all">Все судьи</option>
                          {uniqueJudges.map(judge => <option key={judge} value={judge}>
                              {judge}
                            </option>)}
                        </select>
                      </div>
            ...
            {/* Select All */}
            {filteredCases.length > 0 && <div className="flex items-center gap-3">
                <label className="flex items-center gap-2 px-3 py-2 bg-white rounded-lg border border-claude-border hover:bg-claude-bg transition-colors cursor-pointer">
                  <input type="checkbox" checked={selectedCases.size === filteredCases.length && filteredCases.length > 0} onChange={selectAll} className="w-4 h-4 rounded border-claude-border text-claude-accent focus:ring-claude-accent cursor-pointer" />
                  <span className="text-sm font-medium text-claude-text font-sans">
                    Выбрать все
                  </span>
                </label>
                {selectedCases.size > 0 && <span className="text-sm text-claude-subtext font-sans">
        ...
        <div className={viewMode === 'compact' ? 'space-y-2' : 'space-y-3'}>
          {filteredCases.map((caseItem, index) => <motion.div key={caseItem.id} initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
        ...
        }} onClick={() => onSelectCase?.(caseItem)} className={`group bg-white rounded-2xl border border-claude-border shadow-sm hover:shadow-md hover:border-claude-subtext/30 transition-all cursor-pointer ${viewMode === 'compact' ? 'p-3' : 'p-5'}`}>
              <div className="flex items-start gap-3">
                {/* Checkbox - Only show when select all is active */}
                {isSelectAllActive && <div className="pt-1" onClick={e => e.stopPropagation()}>
                    <input type="checkbox" checked={selectedCases.has(caseItem.id)} onChange={e => toggleCaseSelection(caseItem.id, e)} className="w-4 h-4 rounded border-claude-border text-claude-accent focus:ring-claude-accent cursor-pointer" />
                  </div>}

                {/* Case Icon */}
                {viewMode === 'comfortable' && <div className="w-12 h-12 rounded-xl bg-claude-sidebar border-2 border-white shadow-sm flex items-center justify-center flex-shrink-0">
                  ...
                  <div className="flex items-start justify-between gap-4 mb-2">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`px-2 py-0.5 rounded font-medium border font-sans ${statusConfig[caseItem.status].color} ${viewMode === 'compact' ? 'text-[10px]' : 'text-xs'}`}>
                          {statusConfig[caseItem.status].label}
                        </span>
                      </div>
                      <p className={`text-claude-text font-sans font-medium ${viewMode === 'compact' ? 'text-xs' : 'text-sm'}`}>
                        {caseItem.title}
                      </p>
                      {viewMode === 'comfortable' && <p className="text-xs text-claude-subtext font-sans mt-1">
                          {caseItem.category} • {caseItem.court}
                        </p>}
                    </div>

                    <button onClick={e => e.stopPropagation()} className="p-2 text-claude-subtext hover:text-claude-text hover:bg-claude-bg rounded-lg transition-colors opacity-0 group-hover:opacity-100">
                      <MoreVertical size={16} />
                    </button>
                  </div>

                  {/* Case Details */}
                  <div className={`grid gap-2 ${viewMode === 'compact' ? 'grid-cols-3 text-xs' : 'grid-cols-1 md:grid-cols-3 text-sm mb-4'}`}>
                    <div className="flex items-center gap-1.5 text-claude-subtext">
                      <User size={viewMode === 'compact' ? 12 : 14} className="flex-shrink-0" />
                      <span className="truncate font-sans">
                        {caseItem.client}
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 text-claude-subtext">
                      <Scale size={viewMode === 'compact' ? 12 : 14} className="flex-shrink-0" />
                      <span className="truncate font-sans">
                        {caseItem.judge}
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 text-claude-subtext">
                      <Calendar size={viewMode === 'compact' ? 12 : 14} className="flex-shrink-0" />
                      <span className="font-sans">
                        {new Date(caseItem.nextHearing).toLocaleDateString('ru-RU')}
                      </span>
                    </div>
                  </div>

                  {/* Quick Actions */}
                  {viewMode === 'comfortable' && <div className="flex items-center gap-2 pt-3 border-t border-claude-border/50">
                      <button onClick={e => e.stopPropagation()} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium font-sans text-claude-text hover:bg-claude-bg rounded-lg transition-colors">
                        <Eye size={14} />
                        Просмотр
                      </button>
                      <button onClick={e => e.stopPropagation()} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium font-sans text-claude-text hover:bg-claude-bg rounded-lg transition-colors">
                      ...
                      <button onClick={e => e.stopPropagation()} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium font-sans text-red-600 hover:bg-red-50 rounded-lg transition-colors ml-auto">
                        <Trash2 size={14} />
                        Удалить
                      </button>
                    </div>}
        ...
        {filteredCases.length === 0 && <motion.div initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      ...
      {/* Floating Action Bar */}
      <AnimatePresence>
        {selectedCases.size > 0 && <motion.div initial={{
        y: 100,
        opacity: 0
      }} animate={{
        y: 0,
