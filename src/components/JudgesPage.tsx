...
import { Search, Scale, ChevronRight, Gavel, Percent, Clock, Filter, LayoutGrid, List } from 'lucide-react';
interface Judge {
  id: string;
  name: string;
  court: string;
...
export function JudgesPage({
  ...
  const filteredJudges = judgesData.filter(judge => judge.name.toLowerCase().includes(searchQuery.toLowerCase()) || judge.court.toLowerCase().includes(searchQuery.toLowerCase()));
  return <div className="flex-1 h-full overflow-y-auto bg-claude-bg p-4 md:p-8 lg:p-12">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        ease: [0.22, 1, 0.36, 1]
          ...
          <div className="flex gap-3">
            <div className="relative flex-1 group">
              ...
              <input type="text" className="block w-full pl-11 pr-4 py-4 bg-white border border-claude-border rounded-xl text-claude-text placeholder-claude-subtext/50 focus:outline-none focus:ring-2 focus:ring-claude-accent/20 focus:border-claude-accent transition-all shadow-sm font-sans" placeholder="Поиск по фамилии судьи или названию суда..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
        ...
        <div className={`grid ${viewMode === 'compact' ? 'grid-cols-1 gap-2' : 'grid-cols-1 md:grid-cols-2 gap-4'}`}>
          {filteredJudges.map((judge, index) => <motion.div key={judge.id} initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          delay: index * 0.05 + 0.2
        }} onClick={() => onSelectJudge?.(judge)} className={`group bg-white rounded-2xl border border-claude-border shadow-sm hover:shadow-md hover:border-claude-subtext/30 transition-all cursor-pointer relative overflow-hidden ${viewMode === 'compact' ? 'p-3' : 'p-5'}`}>
              ...
              <div className={`flex items-start gap-3 ${viewMode === 'compact' ? 'mb-2' : 'mb-4'}`}>
                {viewMode === 'comfortable' && <div className="w-14 h-14 rounded-full bg-claude-sidebar border-2 border-white shadow-sm flex items-center justify-center text-xl font-serif text-claude-subtext flex-shrink-0">
                    {judge.name.split(' ').map(n => n[0]).slice(0, 2).join('')}
                  </div>}
                <div className="flex-1">
                  ...
                  <p className={`text-claude-subtext font-sans mt-0.5 line-clamp-1 ${viewMode === 'compact' ? 'text-xs' : 'text-sm'}`}>
                    {judge.court}
                  </p>
                  <div className={`mt-2 inline-flex items-center px-2 py-0.5 rounded font-medium font-sans bg-claude-bg text-claude-subtext border border-claude-border/50 ${viewMode === 'compact' ? 'text-[10px]' : 'text-xs'}`}>
                    {judge.specialization}
                  </div>
                </div>
              </div>

              <div className={`grid grid-cols-3 gap-2 pt-3 border-t border-claude-border/50 ${viewMode === 'compact' ? 'text-xs' : ''}`}>
                <div className="text-center">
                  <div className={`flex items-center justify-center gap-1 text-claude-subtext mb-1 font-sans ${viewMode === 'compact' ? 'text-[10px]' : 'text-xs'}`}>
                    <Gavel size={viewMode === 'compact' ? 10 : 12} />
                    <span>Дел</span>
                  </div>
                  <div className={`font-medium text-claude-text font-serif ${viewMode === 'compact' ? 'text-base' : 'text-lg'}`}>
                    {judge.cases}
                  </div>
                </div>
        ...
        {filteredJudges.length === 0 && <motion.div initial={{
        opacity: 0
      }} animate={{
        opacity: 1
