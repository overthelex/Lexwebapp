...
import { ArrowLeft, Mail, MessageSquare, Send, Users, FileText, Image, Paperclip } from 'lucide-react';
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        delay: 0.1
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => setMessageType('email')} className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all ${messageType === 'email' ? 'border-claude-accent bg-claude-accent/5' : 'border-claude-border hover:border-claude-subtext/30'}`}>
              <div className={`p-2 rounded-lg ${messageType === 'email' ? 'bg-claude-accent text-white' : 'bg-claude-bg text-claude-subtext'}`}>
                <Mail size={20} />
              </div>
            ...
            <button onClick={() => setMessageType('sms')} className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all ${messageType === 'sms' ? 'border-claude-accent bg-claude-accent/5' : 'border-claude-border hover:border-claude-subtext/30'}`}>
              <div className={`p-2 rounded-lg ${messageType === 'sms' ? 'bg-claude-accent text-white' : 'bg-claude-bg text-claude-subtext'}`}>
        ...
        delay: 0.2
          ...
          <div>
            ...
            <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder={messageType === 'email' ? 'Введите текст письма...' : 'Введите текст SMS (макс. 160 символов)...'} rows={messageType === 'email' ? 10 : 4} maxLength={messageType === 'sms' ? 160 : undefined} className="w-full px-4 py-3 bg-white border border-claude-border rounded-xl text-claude-text placeholder-claude-subtext/50 focus:outline-none focus:ring-2 focus:ring-claude-accent/20 focus:border-claude-accent transition-all resize-none font-sans" />
            {messageType === 'sms' && <div className="text-xs text-claude-subtext font-sans mt-2">
        ...
        delay: 0.3
          ...
          <button onClick={handleSend} disabled={!message.trim() || messageType === 'email' && !subject.trim()} className="flex-1 flex items-center justify-center gap-2 px-6 py-3.5 bg-claude-accent text-white rounded-xl font-medium hover:bg-[#C66345] transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
            <Send size={18} />
          ...
          <button onClick={onBack} className="px-6 py-3.5 bg-white border border-claude-border text-claude-text rounded-xl font-medium hover:bg-claude-bg transition-colors">
            Отмена
          </button>
        </motion.div>
      </div>
