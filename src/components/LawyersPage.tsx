...
import { Search, Briefcase, ChevronRight, Award, Users, Filter, LayoutGrid, List } from 'lucide-react';
interface Lawyer {
  id: string;
  name: string;
  firm: string;
...
export function LawyersPage({
  ...
  const filteredLawyers = lawyersData.filter(lawyer => lawyer.name.toLowerCase().includes(searchQuery.toLowerCase()) || lawyer.firm.toLowerCase().includes(searchQuery.toLowerCase()));
  return <div className="flex-1 h-full overflow-y-auto bg-claude-bg p-4 md:p-8 lg:p-12">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        ease: [0.22, 1, 0.36, 1]
          ...
          <div className="flex gap-3">
            <div className="relative flex-1 group">
              ...
              <input type="text" className="block w-full pl-11 pr-4 py-4 bg-white border border-claude-border rounded-xl text-claude-text placeholder-claude-subtext/50 focus:outline-none focus:ring-2 focus:ring-claude-accent/20 focus:border-claude-accent transition-all shadow-sm font-sans" placeholder="Поиск по фамилии адвоката или названию фирмы..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
        ...
        <div className={`grid ${viewMode === 'compact' ? 'grid-cols-1 gap-2' : 'grid-cols-1 md:grid-cols-2 gap-4'}`}>
          {filteredLawyers.map((lawyer, index) => <motion.div key={lawyer.id} initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
        ...
        }} onClick={() => onSelectLawyer?.(lawyer)} className={`group bg-white rounded-2xl border border-claude-border shadow-sm hover:shadow-md hover:border-claude-subtext/30 transition-all cursor-pointer relative overflow-hidden ${viewMode === 'compact' ? 'p-3' : 'p-5'}`}>
        ...
        {filteredLawyers.length === 0 && <motion.div initial={{
        opacity: 0
      }} animate={{
        opacity: 1
