...
import { Search, Clock, MessageSquare, Trash2, MoreVertical, Calendar, Filter, LayoutGrid, List, FileText, CheckCircle, XCircle } from 'lucide-react';
...
export function HistoryPage({
  ...
  const filteredHistory = historyData.filter(item => item.query.toLowerCase().includes(searchQuery.toLowerCase()) || item.category.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return <div className="flex-1 h-full overflow-y-auto bg-claude-bg p-4 md:p-8 lg:p-12 pb-32">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        ease: [0.22, 1, 0.36, 1]
          ...
          <div className="flex flex-col md:flex-row gap-3">
            <div className="relative flex-1 group">
              ...
              <input type="text" className="block w-full pl-11 pr-4 py-3 bg-white border border-claude-border rounded-xl text-claude-text placeholder-claude-subtext/50 focus:outline-none focus:ring-2 focus:ring-claude-accent/20 focus:border-claude-accent transition-all shadow-sm font-sans" placeholder="Поиск в истории..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
        ...
        <div className="space-y-8">
          {groupedHistory.map(([period, items], groupIndex) => <motion.div key={period} initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          delay: groupIndex * 0.1
              ...
              return <motion.div key={item.id} initial={{
                ...
                duration: 0.3,
                delay: index * 0.05
              }} onClick={() => onSelectHistory?.(item)} className={`group bg-white rounded-xl border border-claude-border shadow-sm hover:shadow-md hover:border-claude-subtext/30 transition-all cursor-pointer ${viewMode === 'compact' ? 'p-3' : 'p-4'}`}>
                      <div className="flex items-start gap-3">
                        {/* Icon */}
                        {viewMode === 'comfortable' && <div className="w-10 h-10 rounded-lg bg-claude-sidebar border border-claude-border flex items-center justify-center flex-shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-3 mb-2">
                            <div className="flex-1">
                              <h3 className={`font-sans font-medium text-claude-text group-hover:text-claude-accent transition-colors line-clamp-2 ${viewMode === 'compact' ? 'text-sm' : 'text-base'}`}>
                                {item.query}
                              </h3>
                              <div className={`flex items-center gap-2 mt-1 ${viewMode === 'compact' ? 'text-xs' : 'text-sm'}`}>
                            ...
                            <div className="flex items-center gap-2">
                              {/* Status Badge */}
                              <div className={`flex items-center gap-1 px-2 py-1 rounded border ${statusConfig[item.status].color} ${viewMode === 'compact' ? 'text-xs' : 'text-sm'}`}>
                                <StatusIcon size={viewMode === 'compact' ? 12 : 14} />
                                {viewMode === 'comfortable' && <span className="font-sans font-medium">
                                    {statusConfig[item.status].label}
                                  </span>}
                              </div>

                              {/* More Button */}
                              <button onClick={e => e.stopPropagation()} className="p-1.5 text-claude-subtext hover:text-claude-text hover:bg-claude-bg rounded-lg transition-colors opacity-0 group-hover:opacity-100">
